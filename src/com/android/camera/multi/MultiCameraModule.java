...
public class MultiCameraModule implements CameraModule, PhotoController {
    ...
    public void printMandatoryStreamCombination(){
        ...
        if (manager != null){
            ...
            try{
                Set<Set<String>> cameraIdSet = manager.getConcurrentCameraIds();
                for (Set<String> set : cameraIdSet){
                    for (String id : set){
                        MandatoryStreamCombination[] combinations =
                                manager.getCameraCharacteristics(id).get(mandatoryStreamsKey);
                        if (combinations != null && combinations.length != 0){
                            for (MandatoryStreamCombination combination : combinations){
                                if (combination != null)
                                    Log.d(TAG,"camera id="+id+" MandatoryStreamCombination="+combination.toString());
                                else
                                    Log.d(TAG,"camera id="+id+" MandatoryStreamCombination= null");
                            }
                        } else {
                            Log.d(TAG,"camera id="+id+" MandatoryStreamCombinations not found");
                        }
